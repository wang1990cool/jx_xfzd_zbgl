<template>
  <el-dialog
    :title="!dataForm.id ? '新增人员配备' : '新增人员配备'"
...
<!--  <el-form-item label="人员姓名" prop="ryxm">
    ...
    <el-form-item label="人员姓名" prop="ryxm">
      <el-select v-model="dataForm.ryxm" label="栏目" placeholder="请选择" @change="selectRyxm">
        <el-option v-for="item in ryxxList" :key="item.label" :label="item.label" :value="item.label" >
        </el-option>
      </el-select>
    </el-form-item>
  ...
  export default {
    ...
    methods: {
        init(id){
          this.visible = true
          this.$http({
            url: this.$http.adornUrl('/dic/zdryxxb/selectByssbmmc'),
            method: 'get'
          }).then(({data}) => {
            this.ryxxList = data.ryxxList
          }).then(() => {
              this.$http({
...
/*      init (id) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/zbgl/rypbmxb/update`),
              method: 'get',
              params: this.$http.adornParams({
